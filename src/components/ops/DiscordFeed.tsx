  ...
  while (s.length > 0) {
    ...
    if (s.startsWith("[")) {
      ...
      if (lblEnd > 1) {
        ...
        if (urlEnd > urlStart) {
          const label = s.slice(1, lblEnd);
          const href  = s.slice(urlStart, urlEnd).trim();
          if (/^https?:\/\//i.test(href)) {
            out.push(<a key={k++} href={href} target="_blank" rel="noopener noreferrer" style={{color:"#5865F2",textDecoration:"underline",wordBreak:"break-all"}}>{label}</a>);
            s = s.slice(urlEnd + 1);
            continue;
          }
        }
    ...
    // ── Plain text — exclude [ so markdown link openers are not consumed ─────
