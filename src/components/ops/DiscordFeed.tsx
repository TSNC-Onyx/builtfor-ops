...
export function DiscordFeed() {
  ...
  return (
    <>
      ...
      {/*
        Floating bubble — fixed to bottom-right corner, always visible on
        authenticated pages (Login renders outside OpsShell so never sees this).
        Uses full CSS token palette — no hardcoded colors except Discord blurple
        on the logo mark itself. Zero border-radius on the button per brand spec.
        Avatar images inside the feed use 50% radius as that is Discord's own
        identity chrome, not BuiltFor UI.
      */}
      <button
        onClick={() => setOpen((o) => !o)}
        ...
        style={{
          ...
          width: "44px",
          height: "44px",
          borderRadius: "var(--radius, 0px)", // resolves to 0px per brand spec
      ...
      >
        {/* Discord logo — blurple is Discord's brand identity, not BuiltFor chrome */}
