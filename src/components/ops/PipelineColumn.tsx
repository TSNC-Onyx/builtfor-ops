...
export function PipelineColumn({
  ...
  return (
    <div
      className="flex flex-col min-w-[210px] w-[210px] flex-shrink-0 h-full"
      onDragOver={e => e.preventDefault()}
      onDrop={() => onDrop(stage)}
    >
      {showHeader && (
        <div className="px-3 py-2.5 mb-2 flex-shrink-0" style={{ borderBottom: `2px solid ${headerBorderColor}` }}>
          <div className="flex items-center justify-between">
            <span
              className="font-mono text-[10px] tracking-[0.14em] uppercase"
              style={{ color: isLost ? "hsl(var(--muted-foreground))" : "hsl(var(--foreground))" }}
            >
              {STAGE_LABELS[stage]}
            </span>
            <span className="font-display text-lg leading-none" style={{ color: "hsl(var(--muted-foreground))" }}>
              {prospects.length}
            </span>
          </div>
        </div>
      )}
      {/* Per-column scrollable list — prevents full-page vertical growth */}
      <div className="flex flex-col gap-1.5 overflow-y-auto flex-1 pr-0.5"
        style={{ maxHeight: showHeader ? "calc(100vh - 220px)" : "calc(100vh - 200px)" }}
      >
        {prospects.length === 0 && (
