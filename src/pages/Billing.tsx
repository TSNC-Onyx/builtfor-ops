...
function subStatusColor(s: string): string {
  const map: Record<string, string> = {
    active: GREEN, trialing: NAVY, paused: STEEL,
    past_due: RUST, unpaid: RUST, canceled: STEEL,
    incomplete: RUST, incomplete_expired: STEEL,
  };
  return map[s] ?? STEEL;
}
...
export default function Billing() {
  ...
  const kpis = useMemo(() => {
    ...
    const setupFeesCollected = allEvents
      .filter((e) => e.event_type === "setup_fee" && (e.status === "succeeded" || e.status === "recorded"))
      .reduce((sum, e) => sum + (e.amount_cents ?? 0), 0);
  ...
  async function handleCopyPaymentLink(view: ClientBillingView) {
    const url = await buildSetupFeePaymentLink({
      ...
      email: view.email,
      amount_cents: view.pricing_tier === "founding" ? PRICING.founding_setup_cents : PRICING.standard_setup_cents,
  ...
  async function handleSubscriptionAction(view: ClientBillingView, action: "cancel" | "pause" | "resume") {
    ...
    result.ok ? toast.success(result.message) : toast.error(result.message, { duration: 8000 });
  }
  ...
  return (
    <OpsShell>
      ...
      <div className="px-4 md:px-6 py-4 grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-2" style={{ borderBottom: "1px solid hsl(var(--border))" }}>
        ...
        <BillingKpi label="Paused" value={kpis.pausedSubs.length} color={kpis.pausedSubs.length > 0 ? STEEL : undefined} />
          ...
          {filtered.length === 0 && (
            <div className="p-10 text-center" style={{ border: "1px dashed hsl(var(--border))" }}>
              <div className="font-mono text-[10px] tracking-[0.14em] uppercase" style={{ color: "hsl(var(--muted-foreground))" }}>
                {clients.length === 0 ? "No clients yet — convert a won prospect from Pipeline" : "No results match search"}
              </div>
            </div>
          )}
          {filtered.map((view) => (
...
function BillingKpi({ label, value, color, alert }: { label: string; value: string | number; color?: string; alert?: boolean }) {
  return (
    <div style={{ backgroundColor: "hsl(var(--surface-raised))", border: alert ? "1px solid hsl(var(--rust) / 0.4)" : "1px solid hsl(var(--surface-border))", padding: "12px 14px" }}>
...
function ClientBillingRow({ view, onSelect }: { view: ClientBillingView; onSelect: () => void }) {
  ...
  return (
    <div onClick={onSelect} className="flex items-center gap-3 md:gap-5 px-4 py-3 cursor-pointer hover:opacity-80 transition-opacity"
      ...
      <span className="hidden sm:inline font-mono text-[9px] tracking-[0.1em] uppercase px-2 py-1 flex-shrink-0"
      ...
      {subStatus ? (
        <span className="font-mono text-[9px] tracking-[0.1em] uppercase px-2 py-1 flex-shrink-0"
          style={{ border: `1px solid ${subStatusColor(subStatus)}`, color: subStatusColor(subStatus) }}>
          {fmt(subStatus)}
        </span>
      ) : (
        <span className="hidden md:inline font-mono text-[9px] tracking-[0.1em] uppercase px-2 py-1 flex-shrink-0"
          style={{ border: "1px solid hsl(var(--border))", color: "hsl(var(--muted-foreground))" }}>No Sub</span>
      )}
      <span className="hidden md:inline font-display text-[16px] flex-shrink-0" style={{ color: "hsl(var(--foreground))" }}>
        {sub && (sub.status === "active" || sub.status === "trialing") ? formatCents(sub.amount_cents) : "—"}
...
function ClientBillingDetail({ view, actionLoading, onCopyLink, onOpenLink, onAction }: {
  view: ClientBillingView; actionLoading: boolean;
  onCopyLink: () => void; onOpenLink: () => void;
  ...
  return (
    <div className="space-y-5">
      <section>
        ...
        {sub ? (
          <div className="p-3 space-y-2" style={{ backgroundColor: "hsl(var(--surface-raised))", border: "1px solid hsl(var(--surface-border))" }}>
            ...
            {sub.cancel_at_period_end && <DetailRow label="Cancels" value="At period end" />}
            {sub.stripe_subscription_id && (
              <DetailRow label="Stripe ID">
                <a href={`https://dashboard.stripe.com/subscriptions/${sub.stripe_subscription_id}`}
                  target="_blank" rel="noreferrer" className="font-mono text-[10px]"
      ...
      {sub && (
        <section>
          <div className="font-mono text-[9px] tracking-[0.16em] uppercase mb-2" style={{ color: "hsl(var(--muted-foreground))" }}>Actions</div>
          <div className="flex flex-wrap gap-2">
            ...
            {sub.stripe_subscription_id && (
              <ActionBtn label="Open in Stripe" color={NAVY}
      ...
      <section>
        ...
        <div className="p-3 flex items-center justify-between gap-3"
          style={{ backgroundColor: "hsl(var(--surface-raised))", border: view.setup_fee_paid ? `1px solid ${GREEN}33` : `1px solid ${RUST}33` }}>
          <div>
      ...
      <section>
        ...
        {view.billing_events.length === 0 ? (
          ...
          <div className="space-y-1">
            {view.billing_events.map((e) => <BillingEventRow key={e.id} event={e} />)}
          </div>
        )}
        {view.billing_events.length > 0 && (
          <div className="mt-2 px-2 py-1.5 flex items-center justify-between"
            ...
            <span className="font-display text-[18px]" style={{ color: "hsl(var(--foreground))" }}>{formatCents(view.total_collected_cents)}</span>
          </div>
        )}
      </section>
    </div>
...
function BillingEventRow({ event }: { event: BillingEvent }) {
  return (
    <div className="flex items-center gap-3 px-3 py-2"
      ...
      <span className="font-display text-[16px] flex-shrink-0" style={{ color: "hsl(var(--foreground))" }}>{formatCentsDecimal(event.amount_cents)}</span>
      <span className="flex-1 font-body text-[11px] truncate" style={{ color: "hsl(var(--muted-foreground))" }}>{event.description ?? ""}</span>
      <span className="font-mono text-[9px] flex-shrink-0" style={{ color: "hsl(var(--muted-foreground))", opacity: 0.7 }}>{formatDate(event.created_at)}</span>
    </div>
  );
}
