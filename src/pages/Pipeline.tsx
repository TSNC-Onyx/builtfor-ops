...
export default function Pipeline() {
  ...
  const byStage = STAGE_ORDER.reduce<Record<ProspectStage, Prospect[]>>((acc, s) => { acc[s] = prospects.filter(p => p.stage === s); return acc; }, {} as Record<ProspectStage, Prospect[]>);
  ...
  if (isLoading) return <OpsShell><div className="flex items-center justify-center h-64"><span className="font-mono text-[11px] tracking-[0.14em] uppercase animate-pulse" style={{ color: "hsl(var(--muted-foreground))" }}>Loading pipeline…</span></div></OpsShell>;
  if (isError) return <OpsShell><div className="flex items-center justify-center h-64"><span className="font-mono text-[11px] tracking-[0.14em] uppercase text-rust">Error loading prospects</span></div></OpsShell>;
        ...
        <div className="flex items-center gap-2">
          <div className="flex" style={{ border: "1px solid hsl(var(--border))" }}>
            {(["active","all"] as const).map(v => (
              <button key={v} onClick={() => setViewMode(v)} className="font-mono text-[10px] tracking-[0.12em] uppercase px-3 py-1.5 transition-colors"
          ...
          <button onClick={() => setShowAdd(true)} className="font-mono text-[10px] tracking-[0.12em] uppercase px-4 py-1.5 hover:opacity-90"
            style={{ backgroundColor: "hsl(var(--rust))", color: "hsl(var(--off-white))" }}>+ Add prospect</button>
        </div>
      </div>
      <div className="flex h-[calc(100vh-185px)] overflow-hidden">
        <div className="flex-1 overflow-x-auto">
          <div className="flex gap-2 p-4 h-full">
            {visibleStages.map(stage => <PipelineColumn key={stage} stage={stage} prospects={byStage[stage]} onCardClick={p => setSelected(p)} onDrop={handleDrop} onDragStart={p => { dragRef.current = p; }} />)}
          </div>
        </div>
        {selected && <div className="w-80 flex-shrink-0 overflow-y-auto"><ProspectDetail prospect={selected} onClose={() => setSelected(null)} /></div>}
      </div>
