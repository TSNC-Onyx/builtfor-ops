...
export default function Clients() {
  ...
  async function handleProvision(c: ClientWithSubscription) {
    if (!provisionConfirm) { setProvisionConfirm(true); return; }
    setProvisionConfirm(false);
    const result = await provision(c.id);
    if (result.ok) {
      toast.success(`Tenant provisioned for ${c.business_name}`);
    ...
    <OpsShell>
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2">
                  ...
                  {c.tenant_id && (
                    <span className="hidden md:inline font-mono text-[8px] tracking-[0.1em] uppercase px-1.5 py-0.5 flex-shrink-0"
                      style={{ border: `1px solid ${GREEN}66`, color: GREEN }}>
                      provisioned
                    </span>
                  )}
                </div>
      ...
      {selectedClient && (
        <DrillDownPanel
          ...
          onClose={() => { setSelectedClient(null); setSelectedTier(null); setInviteConfirm(false); setProvisionConfirm(false); }}
        >
          <div className="space-y-4">
            ...
            <ProvisionPanel
              ...
              onProvision={() => handleProvision(selectedClient)}
              onCancelConfirm={() => setProvisionConfirm(false)}
            />
...
function ProvisionPanel({
  ...
  const isProvisioned = !!client.tenant_id;

  return (
    <div style={{
      backgroundColor: isProvisioned ? `${GREEN}0f` : `${NAVY}0f`,
      border: `1px solid ${isProvisioned ? GREEN : NAVY}44`,
      padding: "14px 16px",
    }}>
      <div className="flex items-center justify-between mb-3">
        <div className="font-mono text-[9px] tracking-[0.16em] uppercase" style={{ color: "hsl(var(--muted-foreground))" }}>System Provisioning</div>
        <span className="font-mono text-[9px] tracking-[0.1em] uppercase px-2 py-1"
          style={{ border: `1px solid ${isProvisioned ? GREEN : AMBER}`, color: isProvisioned ? GREEN : AMBER }}>
      ...
      {isProvisioned ? (
        ...
        <>
          <p className="font-mono text-[9px] tracking-[0.06em] mb-3" style={{ color: "hsl(var(--muted-foreground))" }}>
            Creates tenant, owner account, and sends password setup email. Requires credentials in tenant_credentials table.
          </p>
          ...
          {confirm ? (
            <div style={{ border: `1px solid ${AMBER}66`, padding: "10px 12px", backgroundColor: `${AMBER}0f` }}>
              <p className="font-mono text-[9px] tracking-[0.08em] mb-3" style={{ color: AMBER }}>
                Provision tenant system for {client.business_name}? This creates their Supabase account and sends a login email to {client.email}.
              </p>
