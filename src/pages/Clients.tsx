      ...
      <div className="px-4 md:px-6 pb-3 space-y-1 mt-1">
        {paginated.map(c => (
          <div
            ...
            className="flex items-center gap-3 md:gap-5 px-4 py-3 cursor-pointer transition-opacity hover:opacity-80"
          ...
          >
            ...
            <span className="hidden md:inline font-mono text-[9px] tracking-[0.1em] uppercase flex-shrink-0" style={{ color: "hsl(var(--muted-foreground))" }}>{c.pricing_tier}</span>
            ...
            <span className="font-mono text-[9px] flex-shrink-0" style={{ color: "hsl(var(--rust))" }}>→</span>
          </div>
        ))}
      </div>
      ...
      {selectedClient && (
        <DrillDownPanel title={selectedClient.business_name} onClose={() => setSelectedClient(null)}>
          <div className="space-y-3">
            ...
            <DetailRow label="Pricing tier" value={selectedClient.pricing_tier} />
            <DetailRow label="MRR" value={`$${selectedClient.pricing_tier === "founding" ? 300 : 400}/mo`} />
            <DetailRow label="Health" value={selectedClient.health_signal || "—"} />
...
function DetailRow({ label, value, children }: { label: string; value?: string; children?: React.ReactNode }) {
  return (
    <div className="flex items-start gap-3">
      <span className="font-mono text-[9px] tracking-[0.14em] uppercase w-24 flex-shrink-0 pt-0.5" style={{ color: "hsl(var(--muted-foreground))" }}>{label}</span>
